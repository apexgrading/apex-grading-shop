...
export async function listCards({ category, grade, sort = "newest", search = "", page = 1, includeSold = false, soldOnly = false, isGraded = null, isPreorder = null, sinceDays = null } = {}) {
  ...
  if (isGraded !== null) query = query.eq("is_graded", isGraded);
  if (isPreorder !== null) query = query.eq("is_preorder", isPreorder);
...
export async function createCard({ title, category, grade, cert, price, imageUrl, imageUrlBack = null, isGraded = true, condition = null, isPreorder = false, expectedDate = null }) {
