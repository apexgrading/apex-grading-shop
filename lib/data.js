...
export function listCards({ category, grade, sort = "newest", search = "", page = 1, includeSold = false, soldOnly = false } = {}) {
  const where = [];
  const params = [];

  if (soldOnly) where.push("sold = 1");
  else if (!includeSold) where.push("sold = 0");
