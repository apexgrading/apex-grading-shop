...
function cardRow(r) {
  ...
  return {
    ...
    initialQuantity: r.initial_quantity != null ? r.initial_quantity : 1,
    isStockItem: (r.initial_quantity != null ? r.initial_quantity : 1) > 1,
...
export async function listCards({ category, grade, sort = "newest", search = "", page = 1, includeSold = false, soldOnly = false, isGraded = null, isPreorder = null, sinceDays = null, excludeCategory = null, condition = null, excludeCondition = null, minPrice = null, maxPrice = null, setName = null, language = null } = {}) {
  ...
  function buildQuery() {
    ...
    if (setName !== null) q = q.eq("set_name", setName);
    if (language !== null) q = q.eq("language", language);
    if (excludeCondition !== null) q = q.neq("condition", excludeCondition);
    ...
    if (search) {
      // Strip characters PostgREST's filter syntax treats as special
      // (comma separates OR conditions, parentheses group them) so a search
      // term can't inject additional filter clauses.
      const safeSearch = search.replace(/[,()]/g, " ").trim();
      if (safeSearch) q = q.or(`title.ilike.%${safeSearch}%,cert.ilike.%${safeSearch}%`);
    }
    ...
    else if (sort === "a-z") q = q.order("title", { ascending: true });
...
export async function createCard({ title, category, grade, cert, price, imageUrl, imageUrlBack = null, isGraded = true, condition = null, isPreorder = false, expectedDate = null, quantity = 1, setName = null, language = "English" }) {
...
export async function markOrderPaid(orderId, email) {
  ...
  for (const item of items || []) {
    ...
    // Only auto-hide (sold=true) items that were always one-of-a-kind. A
    // multi-unit stock item reaching 0 stays listed — it shows "Out of Stock"
    // on the frontend instead of disappearing, since it might get restocked.
    const wasUnique = (cardRow.initial_quantity || 1) <= 1;
    const { error: soldError } = await supabase
      .from("cards")
      .update({ quantity: remaining, sold: wasUnique && remaining <= 0 })
      .eq("id", item.card_id);
...
export async function updateCardQuantity(id, quantity) {
  const { data: card, error: fetchError } = await supabase
    ...
    .eq("id", id)
    .maybeSingle();
  throwIfError(fetchError, "updateCardQuantity fetch");
  if (!card) throw new Error("Card not found");

  // Restocking (quantity > 0) always makes it available again. Setting it
  // back to 0 only auto-marks sold=true if it was always a unique item —
  // stock items stay listed as Out of Stock instead.
  const wasUnique = (card.initial_quantity || 1) <= 1;
  const sold = quantity > 0 ? false : wasUnique;

  const { error } = await supabase.from("cards").update({ quantity, sold }).eq("id", id);
  throwIfError(error, "updateCardQuantity");
}

// ---------- Site settings (simple key-value toggles) ----------
...
// Returns every distinct set name currently in use, for populating the Set
// filter dropdown - only sets that actually have listings show up.
export async function listDistinctSets(category = null) {
  let q = supabase.from("cards").select("set_name").not("set_name", "is", null).eq("sold", false);
  if (category) q = q.eq("category", category);
  const { data, error } = await q;
  throwIfError(error, "listDistinctSets");
  const unique = [...new Set((data || []).map((r) => r.set_name).filter(Boolean))];
  return unique.sort();
}
